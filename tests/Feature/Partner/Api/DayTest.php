...
describe('Day Controller tests', function () {
    ...
    it('returns 404 for non-existent activity when fetching days', function () {
        $this->getJson(route('partner.day.all', ['owner_type' => 'activity', 'owner_id' => 9999]))->assertStatus(404);
    });
    ...
    it('finds a specific day by ID', function () {
        ...
        $response = $this->getJson(route(
            'partner.day.find',
            ['owner_type' => 'activity', 'owner_id' => $activity->id, 'day_id' => $day->id]
        ))
            ->assertOk();
    ...
    it('returns 404 when finding a day with invalid activity and day ID', function () {
        $this->getJson(route('partner.day.find', ['owner_type' => 'activity', 'owner_id' => 90909, 'day_id' => 9999]))->assertStatus(404);
    });
    ...
    it('creates a new day for an activity', function () {
        ...
        $response = $this->postJson(route('partner.day.create', ['owner_type' => 'activity', 'owner_id' => $activity->id]), $dayData)
            ->assertOk();
    ...
    it('creates multiple days for an activity', function () {
        ...
        $response = $this->postJson(route('partner.day.createMany', ['owner_type' => 'activity', 'owner_id' => $activity->id]), ['days' => $days])
            ->assertOk();
    ...
    it('cant creates more that 7 days for activity', function () {
        ...
        $activity->days()->delete();
        $this->postJson(
            route('partner.day.createMany', ['owner_type' => 'activity', 'owner_id' => $activity->id]),
    ...
    it('fails to create a day with invalid data', function () {
        ...
        $this->postJson(route('partner.day.create', ['owner_type' => 'activity', 'owner_id' => $activity->id]), [])
            ->assertStatus(422);
    });
    ...
    it('fails to create a duplicate day', function () {
        ...
        $this->postJson(route('partner.day.create', ['owner_type' => 'activity', 'owner_id' => $activity->id]), $existingDay)
            ->assertStatus(400);
    });
    ...
    it('updates a specific day', function () {
        ...
        $response = $this->patchJson(
            route('partner.day.update', ['owner_type' => 'activity', 'owner_id' => $activity->id]),
            [
                'day_id' => $day->id,
                ...$updateData,
            ]
    ...
    it('toggles activation status of a day', function () {
        ...
        $this->postJson(route('partner.day.toggleActivation', ['owner_type' => 'activity', 'owner_id' => $activity->id]), [
            'day_id' => $day->id,
        ])->assertOk();
    ...
    it('deletes a specific day', function () {
        ...
        $res = $this->deleteJson(route('partner.day.delete', ['owner_type' => 'activity', 'owner_id' => $activity->id]), [
            'day_id' => $day->id,
        ]);
        $res->assertOk();
