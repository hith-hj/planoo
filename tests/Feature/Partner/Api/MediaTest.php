...
describe('Media Controller Tests', function () {
    it('returns media for a valid activity', function () {
        ...
        $res = $this->getJson(route('partner.media.all', ['owner_type' => 'activity', 'owner_id' => $activity->id]))
            ->assertOk();
    ...
    it('returns 404 for media of an invalid activity', function () {
        $this->getJson(route('partner.media.all', ['owner_type' => 'activity', 'owner_id' => 1000]))->assertStatus(404);
    });

    it('uploads multiple images for an activity', function () {
        $activity = Activity::factory()->for($this->user, 'user')->create();
        $res = $this->postJson(route('partner.media.create', ['owner_type' => 'activity', 'owner_id' => $activity->id]), [
            'type' => 'image',
    ...
    it('cant uploads more than 5 medias', function () {
        $activity = Activity::factory()->for($this->user, 'user')->create();
        $this->postJson(route('partner.media.create', ['owner_type' => 'activity', 'owner_id' => $activity->id]), [
            'type' => 'image',
    ...
    it('deletes media for an activity', function () {
        ...
        foreach ($res->json('payload.medias') as $media) {
            ...
            $this->deleteJson(route('partner.media.delete', ['owner_type' => 'activity', 'owner_id' => $activity->id]), [
                'media_id' => $media['id'],
            ])->assertOk();
